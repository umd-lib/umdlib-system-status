(function (Drupal, drupalSettings) {
  ...
  Drupal.behaviors.systemStatusBehavior = {
    attach: function (context, settings) {
      const systemStatusUrl = drupalSettings.system_status.system_status_url;

      function retrieveStatus(systemStatusUrl) {
        const statusContainer = context.querySelector(
          "#showSystem .navigation__utility_system-status-item",
        );
        ...
        fetch(systemStatusUrl)
          ...
          .then((data) => {
            ...
            if (
              typeof data["non_normal"] === "undefined" ||
              typeof data["non_normal_list"] === "undefined"
            ) {
              console.error(
                "Invalid system status response structure: missing non_normal or non_normal_list",
              );
              return;
            }
            ...
            if (badge) {
              if (nonNormalCount > 0) {
                ...
                badge.setAttribute(
                  "aria-label",
                  `${nonNormalCount} ${nonNormalCount === 1 ? "service" : "services"} experiencing issues`,
                );
                badge.hidden = false;
              } else {
                badge.hidden = true;
            ...
            if (statusSection) {
              statusSection.setAttribute(
                "aria-label",
                `Library system status as of ${formattedDateTime}`,
              );
            }

            // Update list label based on state
            if (nonNormalCount > 0) {
              ...
              statusList.removeAttribute("aria-label");
            }

            // Remove existing error items (all but the first operational item)
            const existingErrors = statusList.querySelectorAll(
              "li.navigation__utility_system-status-item:not(:first-child)",
            );
            ...
            if (nonNormalCount > 0) {
              ...
              data["non_normal_list"].forEach((service) => {
                ...
                errorItem.className =
                  "navigation__utility_system-status-item s-stack-small t-body-small";
                errorItem.innerHTML = `
                  <div class="navigation__utility_system-status-item-status t-body-small">
                    <span class="navigation__utility_system-status-item-icon" aria-hidden="true">
      ...
      if (
        showSystemElement &&
        !showSystemElement.dataset.systemStatusBehavior
      ) {
